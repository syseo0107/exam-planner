...
- [ ] T018 [US2] `index.html`에 오늘 요약 섹션 렌더링 기능 구현
- [ ] T019 [US2] `app.js`에 남은 날짜와 남은 분량을 이용한 오늘 분량 계산 로직 구현
- [ ] T020 [US2] 과목 등록 후와 페이지 로드 시 오늘 분량 요약이 갱신되도록 구현
...
- [ ] T021 [P] [US3] `test.html`에 완료 체크 후 재계산 동작을 검증하는 브라우저 테스트 추가

### Implementation for User Story 3

- [ ] T022 [US3] `index.html`에 각 과목별 완료 체크박스 UI 추가
- [ ] T023 [US3] `app.js`에 완료 처리 핸들러를 구현하여 `remainingVolume`을 수정하고 상태를 저장
- [ ] T024 [US3] 완료 체크 처리 후 오늘 학습량과 과목 요약을 즉시 갱신하도록 구현
...
- [ ] T025 [P] [US4] `test.html`에 하루 건너뛰기 후 재분배 로직을 검증하는 브라우저 테스트 추가

### Implementation for User Story 4

- [ ] T026 [US4] `app.js`에 건너뛴 날을 감지하고 남은 분량을 재분배하는 로직 구현
- [ ] T027 [US4] 재분배 후 오늘 학습량 계산과 렌더링을 갱신하도록 구현
...
- [ ] T028 [P] [US5] `test.html`에 여러 과목 동시 등록 및 표시를 검증하는 브라우저 테스트 추가

### Implementation for User Story 5

- [ ] T029 [US5] `index.html`에 여러 과목 항목을 렌더링하는 기능 구현
- [ ] T030 [US5] `app.js`가 각 과목을 `localStorage`에 독립적으로 저장하고 계산하도록 구현
- [ ] T031 [US5] 페이지 새로고침 후에도 여러 과목 데이터가 분리되어 유지되는지를 확인
...
- [ ] T032 [P] `index.html`과 `app.js`에 비현실적으로 큰 하루 분량에 대한 안내 문구 추가
- [ ] T033 [P] `app.js`에 localStorage 손상 처리 및 초기화 옵션 구현
- [ ] T034 [P] `style.css`에 폼, 요약, 과목 목록의 가독성을 위한 기본 스타일 추가
- [ ] T035 [P] `test.html`에 시험일이 오늘인 경우, 과거인 경우, 자정 경계, 비현실적 분량에 대한 엣지 케이스 테스트 추가
- [ ] T036 [P] `app.js`, `index.html`, `style.css`의 단순성과 유지보수성을 위해 검토 및 정리
- [ ] T037 [P] 브라우저에서 앱이 정상 동작하는지 확인하고 GitHub Pages 배포용 파일 준비
